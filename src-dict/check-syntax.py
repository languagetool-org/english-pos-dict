...
def check_line(line):
  to_ignore = [":=:=all", ";=:=all", "$=$=all", "#=#=all", "\"=''=all", "\"=``=all", "there=EX=all", "to=TO=all"]
  ...
  if "/" not in tags:
    if tags not in ["noun", "verb", "noun_U", "noun_UN", "JJ", "UNTAGGED", 
                    "NN", "NN:U", "NN:UN", "RB", "NNP", "NNS", "UH", "NNPS", "CC", "IN", "CD", "DT", "FW", "PRP",
                    "WP", "WP$", "PRP$", "RBS", "WRB", "WDT", "RP", "MD", "RBR", "POS", "PDT", ""
                    "abbreviation", "contraction", "symbol", "punctuation", "untagged", "VBD"]:
      print (line + " >> unknown tag")
  if len(lemmas)!=len(variants_groups):
    print (line + " >> number of elements in lemma doesn't much number of elements in variant")
  for variants_group in variants_groups:
    variants = variants_group.split(",")
    for variant in variants:
      if variant not in ["none", "all", "us", "gb", "ca", "nz", "au", "za", "us-large"]:
  ...
  for lemma in lemmas:
    ...
    if extra!="" and tags in ["noun", "noun_U", "noun_UN"]:
      if not extra.startswith("pl. "):
        print (line + " >> In nouns, please indicate plural forms [pl. xxxx,yyyy]")
    if extra!="" and tags=="verb" and ";" not in extra:
      ...
      if len(duplicate_consonant)==0:
        print (line + " >> In verbs, please indicate a double consonant [-ll-] or [-ll-,-l-]")
    ...
    if ";" in extra:
      ...
      inflection.parseVerbalForms(lemma+"="+inflection.getInflectedFormsAndTags(lemma, "verb", extra)+"="+parts[2])
