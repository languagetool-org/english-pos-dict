...
def getInflectedFormsAndTags(lemma, pos, extra=""):
    ...
    if pos == "verb":
        ...
        if len(lemma) > 2 and (
            lemma.endswith("x")
            or lemma.endswith("sh")
            or lemma.endswith("ss")
            or lemma.endswith("ch")
            or (lemma.endswith("s") and lemma[-2] in vowels)
        ):
            third_person = "es"
        if ";" in extra:
            forms = lemma + "/VB"
            ...
            forms = (
                ("-/VB,-$ed/VBD,-$ing/VBG,-$ed/VBN,-/VBP,-" + third_person + "/VBZ")
                .replace("-", lemma)
                .replace("$", duplicate_consonant)
            )
        elif len(duplicate_consonant_list) == 2:
            duplicate_consonant = extra[1]
            forms = (
                (
                    "-/VB,-ed/VBD,-$ed/VBD,-ing/VBG,-$ing/VBG,-ed/VBN,-$ed/VBN,-/VBP,-"
                    + third_person
                    + "/VBZ"
                )
                .replace("-", lemma)
            ...
            root = lemma[:-1]
            forms = "-y/VB,-ied/VBD,-ying/VBG,-ied/VBN,-y/VBP,-ies/VBZ".replace(
                "-", root
            )
    ...
    if forms == "":
        if extra != "":
            print(
                "ERROR: cannot generate forms with lemma and pos: "
                + lemma
                + " ["
                + extra
                + "]="
...
def getFormsFromLine(line):
    ...
    for i, lemma in enumerate(lemmas.split("|")):
        ...
        inflected_forms.extend(
            getFormTagLemma(inflected_forms_tags, lemma, variants_groups[i])
        )
    return inflected_forms
...
def isErrorInVerbalForms(tag_forms_dict, lemma):
    ...
    if len(tag_forms_dict["VBP"]) > 1:
        ...
        is_error = True
    if (
        len(tag_forms_dict["VB"]) == 1
        and len(tag_forms_dict["VBP"]) == 1
        and tag_forms_dict["VB"][0] != tag_forms_dict["VBP"][0]
    ):
        print(
            "ERROR: VB and VBP forms are expected to be equal: " + str(tag_forms_dict)
        )
        is_error = True
...
def parseVerbalForms(line):
    ...
    if not isErrorInVerbalForms(tag_forms_dict, lemmas):
        ...
        new_line = (
            lemmas
            + " ["
            + ",".join(tag_forms_dict["VBZ"])
            + ";"
            + ",".join(tag_forms_dict["VBD"])
            + ";"
            + ",".join(tag_forms_dict["VBN"])
            + ";"
            + ",".join(tag_forms_dict["VBG"])
            + "]=verb="
            + variants
        )
        return new_line
...
assert (
    ...
    == "add/VB,added/VBD,adding/VBG,added/VBN,add/VBP,adds/VBZ"
)
assert (
    getInflectedFormsAndTags("wish", "verb")
    == "wish/VB,wished/VBD,wishing/VBG,wished/VBN,wish/VBP,wishes/VBZ"
)
assert (
    getInflectedFormsAndTags("portray", "verb")
    == "portray/VB,portrayed/VBD,portraying/VBG,portrayed/VBN,portray/VBP,portrays/VBZ"
)
assert (
    getInflectedFormsAndTags("specify", "verb")
    == "specify/VB,specified/VBD,specifying/VBG,specified/VBN,specify/VBP,specifies/VBZ"
)
assert (
    getInflectedFormsAndTags("hug", "verb")
    == "hug/VB,hugged/VBD,hugging/VBG,hugged/VBN,hug/VBP,hugs/VBZ"
)
# doesn't work for irregular verbs
assert (
    getInflectedFormsAndTags("run", "verb")
    != "run/VB,ran/VBD,running/VBG,run/VBN,run/VBP,runs/VBZ"
)
