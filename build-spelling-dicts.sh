...
do
	...
	grep -v -Ff ./spell-dicts/prohibit.txt ./spell-dicts/en_${variant}.txt > ./spell-dicts/en_${variant}-2.txt
	mv ./spell-dicts/en_${variant}-2.txt ./spell-dicts/en_${variant}.txt
...
do
	...
	if [ "$my_variable" = "GB" ]; then
		freqlist=./spell-data/freq/en_gb_wrodlist.xml
	fi
	if [ "$my_variable" = "US" ]; then
		freqlist=./spell-data/freq/en_us_wrodlist.xml
	fi
	java -cp $lt_tools org.languagetool.tools.SpellDictionaryBuilder -i ./spell-dicts/en_${variant}.txt -info ${target_dir}/en_${variant}.info -o ${target_dir}/en_${variant}.dict

	#java -cp $jarfile org.languagetool.tools.SpellDictionaryBuilder -i ${targetdict}_spelling.txt -freq $freqlist -info ca-ES_spelling.info -o ${targetdict}_spelling.dict
done
